...
-- esto solo es una copia para que la cargue docker
-- ============================================================
...
GRANT ALL PRIVILEGES ON MrFinanceV2.* TO 'user'@'%';
FLUSH PRIVILEGES;
...
-- ------------------------------------------------------------
-- Trigger: para antes de eliminar una categoria
...
BEGIN
  ...
  IF v_total > 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'No se puede eliminar la categoria: tiene movimientos asociados.';
  END IF;
END$$
...
INSERT INTO usuarios (nombre, email, pass, is_2fa) VALUES
  ('Ana Garcia', 'ana@example.com', '1231', FALSE),
  ('Luis Perez',  'luis@example.com', '123', TRUE),
...
INSERT INTO movimientos (concepto, tipo, monto, id_usuario, id_categoria) VALUES
  ('Nomina marzo',     'ingreso', 1800.00, 1, 3),
